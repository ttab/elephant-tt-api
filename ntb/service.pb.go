// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.12
// 	protoc        (unknown)
// source: ntb/service.proto

package ntb

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type Orientation int32

const (
	Orientation_ORIENTATION_UNSPECIFIED Orientation = 0
	Orientation_ORIENTATION_HORIZONTAL  Orientation = 1
	Orientation_ORIENTATION_VERTICAL    Orientation = 2
	Orientation_ORIENTATION_SQUARE      Orientation = 3
)

// Enum value maps for Orientation.
var (
	Orientation_name = map[int32]string{
		0: "ORIENTATION_UNSPECIFIED",
		1: "ORIENTATION_HORIZONTAL",
		2: "ORIENTATION_VERTICAL",
		3: "ORIENTATION_SQUARE",
	}
	Orientation_value = map[string]int32{
		"ORIENTATION_UNSPECIFIED": 0,
		"ORIENTATION_HORIZONTAL":  1,
		"ORIENTATION_VERTICAL":    2,
		"ORIENTATION_SQUARE":      3,
	}
)

func (x Orientation) Enum() *Orientation {
	p := new(Orientation)
	*p = x
	return p
}

func (x Orientation) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (Orientation) Descriptor() protoreflect.EnumDescriptor {
	return file_ntb_service_proto_enumTypes[0].Descriptor()
}

func (Orientation) Type() protoreflect.EnumType {
	return &file_ntb_service_proto_enumTypes[0]
}

func (x Orientation) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use Orientation.Descriptor instead.
func (Orientation) EnumDescriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{0}
}

type Location int32

const (
	Location_LOCATION_UNSPECIFIED        Location = 0
	Location_LOCATION_IN_GALLERY         Location = 1
	Location_LOCATION_NOT_IN_GALLERY     Location = 2
	Location_LOCATION_IN_COLLECTIONS     Location = 3
	Location_LOCATION_NOT_IN_COLLECTIONS Location = 4
	Location_LOCATION_ALL_ORGANIZATIONS  Location = 5
)

// Enum value maps for Location.
var (
	Location_name = map[int32]string{
		0: "LOCATION_UNSPECIFIED",
		1: "LOCATION_IN_GALLERY",
		2: "LOCATION_NOT_IN_GALLERY",
		3: "LOCATION_IN_COLLECTIONS",
		4: "LOCATION_NOT_IN_COLLECTIONS",
		5: "LOCATION_ALL_ORGANIZATIONS",
	}
	Location_value = map[string]int32{
		"LOCATION_UNSPECIFIED":        0,
		"LOCATION_IN_GALLERY":         1,
		"LOCATION_NOT_IN_GALLERY":     2,
		"LOCATION_IN_COLLECTIONS":     3,
		"LOCATION_NOT_IN_COLLECTIONS": 4,
		"LOCATION_ALL_ORGANIZATIONS":  5,
	}
)

func (x Location) Enum() *Location {
	p := new(Location)
	*p = x
	return p
}

func (x Location) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (Location) Descriptor() protoreflect.EnumDescriptor {
	return file_ntb_service_proto_enumTypes[1].Descriptor()
}

func (Location) Type() protoreflect.EnumType {
	return &file_ntb_service_proto_enumTypes[1]
}

func (x Location) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use Location.Descriptor instead.
func (Location) EnumDescriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{1}
}

type Origin int32

const (
	Origin_ORIGIN_UNSPECIFIED Origin = 0
	Origin_ORIGIN_UPLOAD      Origin = 1
	Origin_ORIGIN_GUEST       Origin = 2
	Origin_ORIGIN_GUESTFTP    Origin = 3
	Origin_ORIGIN_EMAIL       Origin = 4
	Origin_ORIGIN_IMPORT      Origin = 5
	Origin_ORIGIN_FTP         Origin = 6
	Origin_ORIGIN_LTR         Origin = 7
	Origin_ORIGIN_PS          Origin = 8
	Origin_ORIGIN_CMS         Origin = 9
)

// Enum value maps for Origin.
var (
	Origin_name = map[int32]string{
		0: "ORIGIN_UNSPECIFIED",
		1: "ORIGIN_UPLOAD",
		2: "ORIGIN_GUEST",
		3: "ORIGIN_GUESTFTP",
		4: "ORIGIN_EMAIL",
		5: "ORIGIN_IMPORT",
		6: "ORIGIN_FTP",
		7: "ORIGIN_LTR",
		8: "ORIGIN_PS",
		9: "ORIGIN_CMS",
	}
	Origin_value = map[string]int32{
		"ORIGIN_UNSPECIFIED": 0,
		"ORIGIN_UPLOAD":      1,
		"ORIGIN_GUEST":       2,
		"ORIGIN_GUESTFTP":    3,
		"ORIGIN_EMAIL":       4,
		"ORIGIN_IMPORT":      5,
		"ORIGIN_FTP":         6,
		"ORIGIN_LTR":         7,
		"ORIGIN_PS":          8,
		"ORIGIN_CMS":         9,
	}
)

func (x Origin) Enum() *Origin {
	p := new(Origin)
	*p = x
	return p
}

func (x Origin) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (Origin) Descriptor() protoreflect.EnumDescriptor {
	return file_ntb_service_proto_enumTypes[2].Descriptor()
}

func (Origin) Type() protoreflect.EnumType {
	return &file_ntb_service_proto_enumTypes[2]
}

func (x Origin) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use Origin.Descriptor instead.
func (Origin) EnumDescriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{2}
}

type FileType int32

const (
	FileType_FILE_TYPE_UNSPECIFIED FileType = 0
	FileType_FILE_TYPE_IMAGE       FileType = 1
	FileType_FILE_TYPE_VIDEO       FileType = 2
	FileType_FILE_TYPE_PDF         FileType = 3
	FileType_FILE_TYPE_VECTOR      FileType = 4
	FileType_FILE_TYPE_PHOTOSHOP   FileType = 5
)

// Enum value maps for FileType.
var (
	FileType_name = map[int32]string{
		0: "FILE_TYPE_UNSPECIFIED",
		1: "FILE_TYPE_IMAGE",
		2: "FILE_TYPE_VIDEO",
		3: "FILE_TYPE_PDF",
		4: "FILE_TYPE_VECTOR",
		5: "FILE_TYPE_PHOTOSHOP",
	}
	FileType_value = map[string]int32{
		"FILE_TYPE_UNSPECIFIED": 0,
		"FILE_TYPE_IMAGE":       1,
		"FILE_TYPE_VIDEO":       2,
		"FILE_TYPE_PDF":         3,
		"FILE_TYPE_VECTOR":      4,
		"FILE_TYPE_PHOTOSHOP":   5,
	}
)

func (x FileType) Enum() *FileType {
	p := new(FileType)
	*p = x
	return p
}

func (x FileType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (FileType) Descriptor() protoreflect.EnumDescriptor {
	return file_ntb_service_proto_enumTypes[3].Descriptor()
}

func (FileType) Type() protoreflect.EnumType {
	return &file_ntb_service_proto_enumTypes[3]
}

func (x FileType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use FileType.Descriptor instead.
func (FileType) EnumDescriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{3}
}

type TimestampPrecision int32

const (
	TimestampPrecision_TS_PRECISION_UNKNOWN TimestampPrecision = 0
	TimestampPrecision_TS_PRECISION_FULL    TimestampPrecision = 1
	TimestampPrecision_TS_PRECISION_MINUTE  TimestampPrecision = 2
	TimestampPrecision_TS_PRECISION_HOUR    TimestampPrecision = 3
	TimestampPrecision_TS_PRECISION_DAY     TimestampPrecision = 4
	TimestampPrecision_TS_PRECISION_MONTH   TimestampPrecision = 5
	TimestampPrecision_TS_PRECISION_YEAR    TimestampPrecision = 6
	TimestampPrecision_TS_PRECISION_DECADE  TimestampPrecision = 7
	TimestampPrecision_TS_PRECISION_CENTURY TimestampPrecision = 8
)

// Enum value maps for TimestampPrecision.
var (
	TimestampPrecision_name = map[int32]string{
		0: "TS_PRECISION_UNKNOWN",
		1: "TS_PRECISION_FULL",
		2: "TS_PRECISION_MINUTE",
		3: "TS_PRECISION_HOUR",
		4: "TS_PRECISION_DAY",
		5: "TS_PRECISION_MONTH",
		6: "TS_PRECISION_YEAR",
		7: "TS_PRECISION_DECADE",
		8: "TS_PRECISION_CENTURY",
	}
	TimestampPrecision_value = map[string]int32{
		"TS_PRECISION_UNKNOWN": 0,
		"TS_PRECISION_FULL":    1,
		"TS_PRECISION_MINUTE":  2,
		"TS_PRECISION_HOUR":    3,
		"TS_PRECISION_DAY":     4,
		"TS_PRECISION_MONTH":   5,
		"TS_PRECISION_YEAR":    6,
		"TS_PRECISION_DECADE":  7,
		"TS_PRECISION_CENTURY": 8,
	}
)

func (x TimestampPrecision) Enum() *TimestampPrecision {
	p := new(TimestampPrecision)
	*p = x
	return p
}

func (x TimestampPrecision) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (TimestampPrecision) Descriptor() protoreflect.EnumDescriptor {
	return file_ntb_service_proto_enumTypes[4].Descriptor()
}

func (TimestampPrecision) Type() protoreflect.EnumType {
	return &file_ntb_service_proto_enumTypes[4]
}

func (x TimestampPrecision) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use TimestampPrecision.Descriptor instead.
func (TimestampPrecision) EnumDescriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{4}
}

type PreviewType int32

const (
	PreviewType_PREVIEW_UNKNOWN PreviewType = 0
	PreviewType_PREVIEW_IMAGE   PreviewType = 1
	PreviewType_PREVIEW_VIDEO   PreviewType = 2
)

// Enum value maps for PreviewType.
var (
	PreviewType_name = map[int32]string{
		0: "PREVIEW_UNKNOWN",
		1: "PREVIEW_IMAGE",
		2: "PREVIEW_VIDEO",
	}
	PreviewType_value = map[string]int32{
		"PREVIEW_UNKNOWN": 0,
		"PREVIEW_IMAGE":   1,
		"PREVIEW_VIDEO":   2,
	}
)

func (x PreviewType) Enum() *PreviewType {
	p := new(PreviewType)
	*p = x
	return p
}

func (x PreviewType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (PreviewType) Descriptor() protoreflect.EnumDescriptor {
	return file_ntb_service_proto_enumTypes[5].Descriptor()
}

func (PreviewType) Type() protoreflect.EnumType {
	return &file_ntb_service_proto_enumTypes[5]
}

func (x PreviewType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use PreviewType.Descriptor instead.
func (PreviewType) EnumDescriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{5}
}

// LangParameterTranslate controls how text inside HTML lang-tagged elements
// is handled during translation.
type LangParameterTranslate int32

const (
	LangParameterTranslate_LANG_PARAMETER_TRANSLATE_UNSPECIFIED LangParameterTranslate = 0
	// Translate only tags where the lang attribute matches the source language.
	LangParameterTranslate_LANG_PARAMETER_TRANSLATE_ORIGIN LangParameterTranslate = 1
	// Ignore lang attributes and translate all text.
	LangParameterTranslate_LANG_PARAMETER_TRANSLATE_ALL LangParameterTranslate = 2
	// Do not translate any text inside lang-tagged elements.
	LangParameterTranslate_LANG_PARAMETER_TRANSLATE_NONE LangParameterTranslate = 3
)

// Enum value maps for LangParameterTranslate.
var (
	LangParameterTranslate_name = map[int32]string{
		0: "LANG_PARAMETER_TRANSLATE_UNSPECIFIED",
		1: "LANG_PARAMETER_TRANSLATE_ORIGIN",
		2: "LANG_PARAMETER_TRANSLATE_ALL",
		3: "LANG_PARAMETER_TRANSLATE_NONE",
	}
	LangParameterTranslate_value = map[string]int32{
		"LANG_PARAMETER_TRANSLATE_UNSPECIFIED": 0,
		"LANG_PARAMETER_TRANSLATE_ORIGIN":      1,
		"LANG_PARAMETER_TRANSLATE_ALL":         2,
		"LANG_PARAMETER_TRANSLATE_NONE":        3,
	}
)

func (x LangParameterTranslate) Enum() *LangParameterTranslate {
	p := new(LangParameterTranslate)
	*p = x
	return p
}

func (x LangParameterTranslate) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (LangParameterTranslate) Descriptor() protoreflect.EnumDescriptor {
	return file_ntb_service_proto_enumTypes[6].Descriptor()
}

func (LangParameterTranslate) Type() protoreflect.EnumType {
	return &file_ntb_service_proto_enumTypes[6]
}

func (x LangParameterTranslate) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use LangParameterTranslate.Descriptor instead.
func (LangParameterTranslate) EnumDescriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{6}
}

type GetAssignmentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The assignee is identified by their commission code.
	CommissionCode string `protobuf:"bytes,1,opt,name=commission_code,json=commissionCode,proto3" json:"commission_code,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GetAssignmentRequest) Reset() {
	*x = GetAssignmentRequest{}
	mi := &file_ntb_service_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAssignmentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAssignmentRequest) ProtoMessage() {}

func (x *GetAssignmentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAssignmentRequest.ProtoReflect.Descriptor instead.
func (*GetAssignmentRequest) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{0}
}

func (x *GetAssignmentRequest) GetCommissionCode() string {
	if x != nil {
		return x.CommissionCode
	}
	return ""
}

type GetAssignmentResponse struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	PlanningUuid   string                 `protobuf:"bytes,1,opt,name=planning_uuid,json=planningUuid,proto3" json:"planning_uuid,omitempty"`
	AssignmentUuid string                 `protobuf:"bytes,2,opt,name=assignment_uuid,json=assignmentUuid,proto3" json:"assignment_uuid,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GetAssignmentResponse) Reset() {
	*x = GetAssignmentResponse{}
	mi := &file_ntb_service_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAssignmentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAssignmentResponse) ProtoMessage() {}

func (x *GetAssignmentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAssignmentResponse.ProtoReflect.Descriptor instead.
func (*GetAssignmentResponse) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{1}
}

func (x *GetAssignmentResponse) GetPlanningUuid() string {
	if x != nil {
		return x.PlanningUuid
	}
	return ""
}

func (x *GetAssignmentResponse) GetAssignmentUuid() string {
	if x != nil {
		return x.AssignmentUuid
	}
	return ""
}

type ListAssignmentsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListAssignmentsRequest) Reset() {
	*x = ListAssignmentsRequest{}
	mi := &file_ntb_service_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListAssignmentsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListAssignmentsRequest) ProtoMessage() {}

func (x *ListAssignmentsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListAssignmentsRequest.ProtoReflect.Descriptor instead.
func (*ListAssignmentsRequest) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{2}
}

type Assignment struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	PlanningUuid   string                 `protobuf:"bytes,1,opt,name=planning_uuid,json=planningUuid,proto3" json:"planning_uuid,omitempty"`
	AssignmentUuid string                 `protobuf:"bytes,2,opt,name=assignment_uuid,json=assignmentUuid,proto3" json:"assignment_uuid,omitempty"`
	AssigneeUuid   string                 `protobuf:"bytes,3,opt,name=assignee_uuid,json=assigneeUuid,proto3" json:"assignee_uuid,omitempty"`
	AssigneeName   string                 `protobuf:"bytes,4,opt,name=assignee_name,json=assigneeName,proto3" json:"assignee_name,omitempty"`
	CommissionCode string                 `protobuf:"bytes,5,opt,name=commission_code,json=commissionCode,proto3" json:"commission_code,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *Assignment) Reset() {
	*x = Assignment{}
	mi := &file_ntb_service_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Assignment) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Assignment) ProtoMessage() {}

func (x *Assignment) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Assignment.ProtoReflect.Descriptor instead.
func (*Assignment) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{3}
}

func (x *Assignment) GetPlanningUuid() string {
	if x != nil {
		return x.PlanningUuid
	}
	return ""
}

func (x *Assignment) GetAssignmentUuid() string {
	if x != nil {
		return x.AssignmentUuid
	}
	return ""
}

func (x *Assignment) GetAssigneeUuid() string {
	if x != nil {
		return x.AssigneeUuid
	}
	return ""
}

func (x *Assignment) GetAssigneeName() string {
	if x != nil {
		return x.AssigneeName
	}
	return ""
}

func (x *Assignment) GetCommissionCode() string {
	if x != nil {
		return x.CommissionCode
	}
	return ""
}

type ListAssignmentsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Assignments   []*Assignment          `protobuf:"bytes,1,rep,name=assignments,proto3" json:"assignments,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListAssignmentsResponse) Reset() {
	*x = ListAssignmentsResponse{}
	mi := &file_ntb_service_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListAssignmentsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListAssignmentsResponse) ProtoMessage() {}

func (x *ListAssignmentsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListAssignmentsResponse.ProtoReflect.Descriptor instead.
func (*ListAssignmentsResponse) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{4}
}

func (x *ListAssignmentsResponse) GetAssignments() []*Assignment {
	if x != nil {
		return x.Assignments
	}
	return nil
}

type GetItemRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Archive to search in.
	Archive string `protobuf:"bytes,1,opt,name=archive,proto3" json:"archive,omitempty"`
	// ID of the item to get.
	Id            string `protobuf:"bytes,2,opt,name=id,proto3" json:"id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetItemRequest) Reset() {
	*x = GetItemRequest{}
	mi := &file_ntb_service_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetItemRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetItemRequest) ProtoMessage() {}

func (x *GetItemRequest) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetItemRequest.ProtoReflect.Descriptor instead.
func (*GetItemRequest) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{5}
}

func (x *GetItemRequest) GetArchive() string {
	if x != nil {
		return x.Archive
	}
	return ""
}

func (x *GetItemRequest) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

type GetItemResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Item          *MediaItem             `protobuf:"bytes,1,opt,name=item,proto3" json:"item,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetItemResponse) Reset() {
	*x = GetItemResponse{}
	mi := &file_ntb_service_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetItemResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetItemResponse) ProtoMessage() {}

func (x *GetItemResponse) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetItemResponse.ProtoReflect.Descriptor instead.
func (*GetItemResponse) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{6}
}

func (x *GetItemResponse) GetItem() *MediaItem {
	if x != nil {
		return x.Item
	}
	return nil
}

type SearchRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Archive to search in.
	Archive string `protobuf:"bytes,1,opt,name=archive,proto3" json:"archive,omitempty"`
	// Search limit, max 200, default: 50.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Offset - search offset, max 10000, default: 0.
	Offset int32 `protobuf:"varint,3,opt,name=offset,proto3" json:"offset,omitempty"`
	// Sort options to the list.
	Sort *Sort `protobuf:"bytes,4,opt,name=sort,proto3" json:"sort,omitempty"`
	// Query - freetext query.
	Query string `protobuf:"bytes,5,opt,name=query,proto3" json:"query,omitempty"`
	// ItemId - filter by the itemId.
	ItemId string `protobuf:"bytes,6,opt,name=item_id,json=itemId,proto3" json:"item_id,omitempty"`
	// City filter.
	City string `protobuf:"bytes,7,opt,name=city,proto3" json:"city,omitempty"`
	// Country filter.
	Country string `protobuf:"bytes,8,opt,name=country,proto3" json:"country,omitempty"`
	// Creator filter (photographer).
	Creator string `protobuf:"bytes,9,opt,name=creator,proto3" json:"creator,omitempty"`
	// Ids filters by asset ids.
	Ids []string `protobuf:"bytes,10,rep,name=ids,proto3" json:"ids,omitempty"`
	// Keywords filter.
	Keywords []string `protobuf:"bytes,11,rep,name=keywords,proto3" json:"keywords,omitempty"`
	// Filter on collections.
	Collections []int32 `protobuf:"varint,12,rep,packed,name=collections,proto3" json:"collections,omitempty"`
	// Filter on albums.
	Albums []int32 `protobuf:"varint,13,rep,packed,name=albums,proto3" json:"albums,omitempty"`
	// Filter on categories.
	Categories []string `protobuf:"bytes,14,rep,name=categories,proto3" json:"categories,omitempty"`
	// Include/exclude assets based on location.
	Locations string `protobuf:"bytes,15,opt,name=locations,proto3" json:"locations,omitempty"`
	// Filter on jobId.
	JobId string `protobuf:"bytes,16,opt,name=job_id,json=jobId,proto3" json:"job_id,omitempty"`
	// Filter on image orientation.
	Orientation Orientation `protobuf:"varint,17,opt,name=orientation,proto3,enum=ttab.ntb.Orientation" json:"orientation,omitempty"`
	// View results after this point in list.
	SearchAfter []string `protobuf:"bytes,18,rep,name=search_after,json=searchAfter,proto3" json:"search_after,omitempty"`
	// Filter on origins.
	Origins []Origin `protobuf:"varint,19,rep,packed,name=origins,proto3,enum=ttab.ntb.Origin" json:"origins,omitempty"`
	// Filter on file type.
	FileType FileType `protobuf:"varint,20,opt,name=file_type,json=fileType,proto3,enum=ttab.ntb.FileType" json:"file_type,omitempty"`
	// Get assets created on and after the RFC3339 timestamp.
	CreatedMin string `protobuf:"bytes,21,opt,name=created_min,json=createdMin,proto3" json:"created_min,omitempty"`
	// Get assets created on and before the RFC3339 timestamp.
	CreatedMax string `protobuf:"bytes,22,opt,name=created_max,json=createdMax,proto3" json:"created_max,omitempty"`
	// Set to true to search against items outside of your subscription agreement.
	OutsideSubscription bool `protobuf:"varint,23,opt,name=outside_subscription,json=outsideSubscription,proto3" json:"outside_subscription,omitempty"`
	// Filter by editorial topics.
	EditorialTopics []string `protobuf:"bytes,24,rep,name=editorial_topics,json=editorialTopics,proto3" json:"editorial_topics,omitempty"`
	// Filter by distributor names.
	DistributorNames []string `protobuf:"bytes,25,rep,name=distributor_names,json=distributorNames,proto3" json:"distributor_names,omitempty"`
	// Filter by events.
	Events []string `protobuf:"bytes,26,rep,name=events,proto3" json:"events,omitempty"`
	// Filter by subject terms.
	SubjectTerms []string `protobuf:"bytes,27,rep,name=subject_terms,json=subjectTerms,proto3" json:"subject_terms,omitempty"`
	// Filter by persons.
	Persons       []string `protobuf:"bytes,28,rep,name=persons,proto3" json:"persons,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SearchRequest) Reset() {
	*x = SearchRequest{}
	mi := &file_ntb_service_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchRequest) ProtoMessage() {}

func (x *SearchRequest) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SearchRequest.ProtoReflect.Descriptor instead.
func (*SearchRequest) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{7}
}

func (x *SearchRequest) GetArchive() string {
	if x != nil {
		return x.Archive
	}
	return ""
}

func (x *SearchRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *SearchRequest) GetOffset() int32 {
	if x != nil {
		return x.Offset
	}
	return 0
}

func (x *SearchRequest) GetSort() *Sort {
	if x != nil {
		return x.Sort
	}
	return nil
}

func (x *SearchRequest) GetQuery() string {
	if x != nil {
		return x.Query
	}
	return ""
}

func (x *SearchRequest) GetItemId() string {
	if x != nil {
		return x.ItemId
	}
	return ""
}

func (x *SearchRequest) GetCity() string {
	if x != nil {
		return x.City
	}
	return ""
}

func (x *SearchRequest) GetCountry() string {
	if x != nil {
		return x.Country
	}
	return ""
}

func (x *SearchRequest) GetCreator() string {
	if x != nil {
		return x.Creator
	}
	return ""
}

func (x *SearchRequest) GetIds() []string {
	if x != nil {
		return x.Ids
	}
	return nil
}

func (x *SearchRequest) GetKeywords() []string {
	if x != nil {
		return x.Keywords
	}
	return nil
}

func (x *SearchRequest) GetCollections() []int32 {
	if x != nil {
		return x.Collections
	}
	return nil
}

func (x *SearchRequest) GetAlbums() []int32 {
	if x != nil {
		return x.Albums
	}
	return nil
}

func (x *SearchRequest) GetCategories() []string {
	if x != nil {
		return x.Categories
	}
	return nil
}

func (x *SearchRequest) GetLocations() string {
	if x != nil {
		return x.Locations
	}
	return ""
}

func (x *SearchRequest) GetJobId() string {
	if x != nil {
		return x.JobId
	}
	return ""
}

func (x *SearchRequest) GetOrientation() Orientation {
	if x != nil {
		return x.Orientation
	}
	return Orientation_ORIENTATION_UNSPECIFIED
}

func (x *SearchRequest) GetSearchAfter() []string {
	if x != nil {
		return x.SearchAfter
	}
	return nil
}

func (x *SearchRequest) GetOrigins() []Origin {
	if x != nil {
		return x.Origins
	}
	return nil
}

func (x *SearchRequest) GetFileType() FileType {
	if x != nil {
		return x.FileType
	}
	return FileType_FILE_TYPE_UNSPECIFIED
}

func (x *SearchRequest) GetCreatedMin() string {
	if x != nil {
		return x.CreatedMin
	}
	return ""
}

func (x *SearchRequest) GetCreatedMax() string {
	if x != nil {
		return x.CreatedMax
	}
	return ""
}

func (x *SearchRequest) GetOutsideSubscription() bool {
	if x != nil {
		return x.OutsideSubscription
	}
	return false
}

func (x *SearchRequest) GetEditorialTopics() []string {
	if x != nil {
		return x.EditorialTopics
	}
	return nil
}

func (x *SearchRequest) GetDistributorNames() []string {
	if x != nil {
		return x.DistributorNames
	}
	return nil
}

func (x *SearchRequest) GetEvents() []string {
	if x != nil {
		return x.Events
	}
	return nil
}

func (x *SearchRequest) GetSubjectTerms() []string {
	if x != nil {
		return x.SubjectTerms
	}
	return nil
}

func (x *SearchRequest) GetPersons() []string {
	if x != nil {
		return x.Persons
	}
	return nil
}

type Sort struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Field         string                 `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	Descending    bool                   `protobuf:"varint,2,opt,name=descending,proto3" json:"descending,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Sort) Reset() {
	*x = Sort{}
	mi := &file_ntb_service_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Sort) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Sort) ProtoMessage() {}

func (x *Sort) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Sort.ProtoReflect.Descriptor instead.
func (*Sort) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{8}
}

func (x *Sort) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

func (x *Sort) GetDescending() bool {
	if x != nil {
		return x.Descending
	}
	return false
}

type SearchResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Page          *Page                  `protobuf:"bytes,1,opt,name=page,proto3" json:"page,omitempty"`
	Data          []*MediaItem           `protobuf:"bytes,3,rep,name=data,proto3" json:"data,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SearchResponse) Reset() {
	*x = SearchResponse{}
	mi := &file_ntb_service_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchResponse) ProtoMessage() {}

func (x *SearchResponse) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SearchResponse.ProtoReflect.Descriptor instead.
func (*SearchResponse) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{9}
}

func (x *SearchResponse) GetPage() *Page {
	if x != nil {
		return x.Page
	}
	return nil
}

func (x *SearchResponse) GetData() []*MediaItem {
	if x != nil {
		return x.Data
	}
	return nil
}

type Page struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Offset int32                  `protobuf:"varint,1,opt,name=offset,proto3" json:"offset,omitempty"`
	Limit  int32                  `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	Count  int32                  `protobuf:"varint,3,opt,name=count,proto3" json:"count,omitempty"`
	Total  int32                  `protobuf:"varint,4,opt,name=total,proto3" json:"total,omitempty"`
	// SearchAfter can be used for pagination in search results.
	SearchAfter   []string `protobuf:"bytes,5,rep,name=search_after,json=searchAfter,proto3" json:"search_after,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Page) Reset() {
	*x = Page{}
	mi := &file_ntb_service_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Page) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Page) ProtoMessage() {}

func (x *Page) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Page.ProtoReflect.Descriptor instead.
func (*Page) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{10}
}

func (x *Page) GetOffset() int32 {
	if x != nil {
		return x.Offset
	}
	return 0
}

func (x *Page) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *Page) GetCount() int32 {
	if x != nil {
		return x.Count
	}
	return 0
}

func (x *Page) GetTotal() int32 {
	if x != nil {
		return x.Total
	}
	return 0
}

func (x *Page) GetSearchAfter() []string {
	if x != nil {
		return x.SearchAfter
	}
	return nil
}

type MediaItem struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// ID of the item.
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// Type is the content type.
	Type string `protobuf:"bytes,2,opt,name=type,proto3" json:"type,omitempty"`
	// Archive (mb/ntb/tt).
	Archive string `protobuf:"bytes,3,opt,name=archive,proto3" json:"archive,omitempty"`
	// UserID is the user ID of user who uploaded the media.
	UserId string `protobuf:"bytes,4,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	// Restrictions indicates whether there are any restrictions noted on the
	// media.
	Restrictions bool `protobuf:"varint,5,opt,name=restrictions,proto3" json:"restrictions,omitempty"`
	// Headline is a brief synopsis of the caption. Corresponds to IPTC field.
	Headline string `protobuf:"bytes,6,opt,name=headline,proto3" json:"headline,omitempty"`
	// Description is a textual description, including captions, of the media.
	Description string `protobuf:"bytes,7,opt,name=description,proto3" json:"description,omitempty"`
	// AltTextAccessibility is a textual description for accessibility.
	AltTextAccessibility string `protobuf:"bytes,8,opt,name=alt_text_accessibility,json=altTextAccessibility,proto3" json:"alt_text_accessibility,omitempty"`
	// Creator contains the name of the photographer or company/org.
	Creator string `protobuf:"bytes,9,opt,name=creator,proto3" json:"creator,omitempty"`
	// Keywords to express the subject of the image.
	Keywords []string `protobuf:"bytes,10,rep,name=keywords,proto3" json:"keywords,omitempty"`
	// Category corresponds to IPTC category field.
	Category string `protobuf:"bytes,11,opt,name=category,proto3" json:"category,omitempty"`
	// PersonsShown are persons in the image.
	PersonsShown []*Person `protobuf:"bytes,12,rep,name=persons_shown,json=personsShown,proto3" json:"persons_shown,omitempty"`
	// ImageRating is the rating of the image by its user or supplier (0-5).
	// Using optional to distinguish between unset and 0.
	ImageRating *int32 `protobuf:"varint,13,opt,name=image_rating,json=imageRating,proto3,oneof" json:"image_rating,omitempty"`
	// Title provides a short human-readable name.
	Title string `protobuf:"bytes,14,opt,name=title,proto3" json:"title,omitempty"`
	// DateArchived designates the date and time the content was archived.
	DateArchived string `protobuf:"bytes,15,opt,name=date_archived,json=dateArchived,proto3" json:"date_archived,omitempty"`
	// DateCreated designates the date and time the content was created. Not
	// present if the source data is invalid or missing.
	DateCreated *ImpreciseTimestamp `protobuf:"bytes,16,opt,name=date_created,json=dateCreated,proto3" json:"date_created,omitempty"`
	// Country corresponds to IPTC field.
	Country string `protobuf:"bytes,17,opt,name=country,proto3" json:"country,omitempty"`
	// City corresponds to IPTC field.
	City string `protobuf:"bytes,18,opt,name=city,proto3" json:"city,omitempty"`
	// CreditLine corresponds to IPTC field.
	CreditLine string `protobuf:"bytes,19,opt,name=credit_line,json=creditLine,proto3" json:"credit_line,omitempty"`
	// VersionCount is the number of available versions.
	VersionCount int32 `protobuf:"varint,20,opt,name=version_count,json=versionCount,proto3" json:"version_count,omitempty"`
	// Instructions are any number of instructions from the provider or creator.
	Instructions string `protobuf:"bytes,21,opt,name=instructions,proto3" json:"instructions,omitempty"`
	// UsageTerms are rights usage terms.
	UsageTerms string `protobuf:"bytes,22,opt,name=usage_terms,json=usageTerms,proto3" json:"usage_terms,omitempty"`
	// Source is the name of a person or party who has a role in the content
	// supply chain.
	Source string `protobuf:"bytes,23,opt,name=source,proto3" json:"source,omitempty"`
	// Suppliers identifies the most recent supplier of the image.
	Suppliers string `protobuf:"bytes,24,opt,name=suppliers,proto3" json:"suppliers,omitempty"`
	// ImageSupplierImageID is an identifier assigned by the Image Supplier.
	ImageSupplierImageId string `protobuf:"bytes,25,opt,name=image_supplier_image_id,json=imageSupplierImageId,proto3" json:"image_supplier_image_id,omitempty"`
	// JobID is an identifier related to the job for which the image is supplied.
	JobId string `protobuf:"bytes,26,opt,name=job_id,json=jobId,proto3" json:"job_id,omitempty"`
	// File contains file information.
	File *File `protobuf:"bytes,27,opt,name=file,proto3" json:"file,omitempty"`
	// Previews contains preview URLs and dimensions.
	Previews []*Preview `protobuf:"bytes,28,rep,name=previews,proto3" json:"previews,omitempty"`
	// Permissions contains user permissions for this media.
	Permissions *Permissions `protobuf:"bytes,29,opt,name=permissions,proto3" json:"permissions,omitempty"`
	// Links contains related links.
	Links map[string]string `protobuf:"bytes,30,rep,name=links,proto3" json:"links,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// Internal contains internal metadata.
	Internal *Internal `protobuf:"bytes,31,opt,name=internal,proto3" json:"internal,omitempty"`
	// SortValues can be used for pagination in search results. Only available
	// when the MediaItem was returned as part of a search result.
	SortValues    []string `protobuf:"bytes,32,rep,name=sort_values,json=sortValues,proto3" json:"sort_values,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MediaItem) Reset() {
	*x = MediaItem{}
	mi := &file_ntb_service_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MediaItem) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MediaItem) ProtoMessage() {}

func (x *MediaItem) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MediaItem.ProtoReflect.Descriptor instead.
func (*MediaItem) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{11}
}

func (x *MediaItem) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *MediaItem) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *MediaItem) GetArchive() string {
	if x != nil {
		return x.Archive
	}
	return ""
}

func (x *MediaItem) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *MediaItem) GetRestrictions() bool {
	if x != nil {
		return x.Restrictions
	}
	return false
}

func (x *MediaItem) GetHeadline() string {
	if x != nil {
		return x.Headline
	}
	return ""
}

func (x *MediaItem) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *MediaItem) GetAltTextAccessibility() string {
	if x != nil {
		return x.AltTextAccessibility
	}
	return ""
}

func (x *MediaItem) GetCreator() string {
	if x != nil {
		return x.Creator
	}
	return ""
}

func (x *MediaItem) GetKeywords() []string {
	if x != nil {
		return x.Keywords
	}
	return nil
}

func (x *MediaItem) GetCategory() string {
	if x != nil {
		return x.Category
	}
	return ""
}

func (x *MediaItem) GetPersonsShown() []*Person {
	if x != nil {
		return x.PersonsShown
	}
	return nil
}

func (x *MediaItem) GetImageRating() int32 {
	if x != nil && x.ImageRating != nil {
		return *x.ImageRating
	}
	return 0
}

func (x *MediaItem) GetTitle() string {
	if x != nil {
		return x.Title
	}
	return ""
}

func (x *MediaItem) GetDateArchived() string {
	if x != nil {
		return x.DateArchived
	}
	return ""
}

func (x *MediaItem) GetDateCreated() *ImpreciseTimestamp {
	if x != nil {
		return x.DateCreated
	}
	return nil
}

func (x *MediaItem) GetCountry() string {
	if x != nil {
		return x.Country
	}
	return ""
}

func (x *MediaItem) GetCity() string {
	if x != nil {
		return x.City
	}
	return ""
}

func (x *MediaItem) GetCreditLine() string {
	if x != nil {
		return x.CreditLine
	}
	return ""
}

func (x *MediaItem) GetVersionCount() int32 {
	if x != nil {
		return x.VersionCount
	}
	return 0
}

func (x *MediaItem) GetInstructions() string {
	if x != nil {
		return x.Instructions
	}
	return ""
}

func (x *MediaItem) GetUsageTerms() string {
	if x != nil {
		return x.UsageTerms
	}
	return ""
}

func (x *MediaItem) GetSource() string {
	if x != nil {
		return x.Source
	}
	return ""
}

func (x *MediaItem) GetSuppliers() string {
	if x != nil {
		return x.Suppliers
	}
	return ""
}

func (x *MediaItem) GetImageSupplierImageId() string {
	if x != nil {
		return x.ImageSupplierImageId
	}
	return ""
}

func (x *MediaItem) GetJobId() string {
	if x != nil {
		return x.JobId
	}
	return ""
}

func (x *MediaItem) GetFile() *File {
	if x != nil {
		return x.File
	}
	return nil
}

func (x *MediaItem) GetPreviews() []*Preview {
	if x != nil {
		return x.Previews
	}
	return nil
}

func (x *MediaItem) GetPermissions() *Permissions {
	if x != nil {
		return x.Permissions
	}
	return nil
}

func (x *MediaItem) GetLinks() map[string]string {
	if x != nil {
		return x.Links
	}
	return nil
}

func (x *MediaItem) GetInternal() *Internal {
	if x != nil {
		return x.Internal
	}
	return nil
}

func (x *MediaItem) GetSortValues() []string {
	if x != nil {
		return x.SortValues
	}
	return nil
}

type ImpreciseTimestamp struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Time as an RFC3339 timestamp.
	Time string `protobuf:"bytes,1,opt,name=time,proto3" json:"time,omitempty"`
	// ValidTimezone is set to true if the timestamp has a valid timezone, if
	// false the timestamp will use zulu time as a substitute for the unknown
	// timezone.
	ValidTimezone bool `protobuf:"varint,2,opt,name=valid_timezone,json=validTimezone,proto3" json:"valid_timezone,omitempty"`
	// Precision is a hint about the precision of the timestamp.
	Precision     TimestampPrecision `protobuf:"varint,3,opt,name=precision,proto3,enum=ttab.ntb.TimestampPrecision" json:"precision,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ImpreciseTimestamp) Reset() {
	*x = ImpreciseTimestamp{}
	mi := &file_ntb_service_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ImpreciseTimestamp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ImpreciseTimestamp) ProtoMessage() {}

func (x *ImpreciseTimestamp) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ImpreciseTimestamp.ProtoReflect.Descriptor instead.
func (*ImpreciseTimestamp) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{12}
}

func (x *ImpreciseTimestamp) GetTime() string {
	if x != nil {
		return x.Time
	}
	return ""
}

func (x *ImpreciseTimestamp) GetValidTimezone() bool {
	if x != nil {
		return x.ValidTimezone
	}
	return false
}

func (x *ImpreciseTimestamp) GetPrecision() TimestampPrecision {
	if x != nil {
		return x.Precision
	}
	return TimestampPrecision_TS_PRECISION_UNKNOWN
}

type Person struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// PersonName is the name of the person in the image.
	PersonName string `protobuf:"bytes,1,opt,name=person_name,json=personName,proto3" json:"person_name,omitempty"`
	// PersonDescription is the description of the person, often their title.
	PersonDescription string `protobuf:"bytes,2,opt,name=person_description,json=personDescription,proto3" json:"person_description,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *Person) Reset() {
	*x = Person{}
	mi := &file_ntb_service_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Person) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Person) ProtoMessage() {}

func (x *Person) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Person.ProtoReflect.Descriptor instead.
func (*Person) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{13}
}

func (x *Person) GetPersonName() string {
	if x != nil {
		return x.PersonName
	}
	return ""
}

func (x *Person) GetPersonDescription() string {
	if x != nil {
		return x.PersonDescription
	}
	return ""
}

type File struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Size is the size of the file in bytes.
	Size int64 `protobuf:"varint,1,opt,name=size,proto3" json:"size,omitempty"`
	// Height is the height of the media in pixels.
	Height int32 `protobuf:"varint,2,opt,name=height,proto3" json:"height,omitempty"`
	// Width is the width of the media in pixels.
	Width int32 `protobuf:"varint,3,opt,name=width,proto3" json:"width,omitempty"`
	// MimeType is the mimetype of the media.
	MimeType string `protobuf:"bytes,4,opt,name=mime_type,json=mimeType,proto3" json:"mime_type,omitempty"`
	// OriginalFilename is the original name of the file.
	OriginalFilename string `protobuf:"bytes,5,opt,name=original_filename,json=originalFilename,proto3" json:"original_filename,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *File) Reset() {
	*x = File{}
	mi := &file_ntb_service_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *File) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*File) ProtoMessage() {}

func (x *File) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use File.ProtoReflect.Descriptor instead.
func (*File) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{14}
}

func (x *File) GetSize() int64 {
	if x != nil {
		return x.Size
	}
	return 0
}

func (x *File) GetHeight() int32 {
	if x != nil {
		return x.Height
	}
	return 0
}

func (x *File) GetWidth() int32 {
	if x != nil {
		return x.Width
	}
	return 0
}

func (x *File) GetMimeType() string {
	if x != nil {
		return x.MimeType
	}
	return ""
}

func (x *File) GetOriginalFilename() string {
	if x != nil {
		return x.OriginalFilename
	}
	return ""
}

type Preview struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Width is the width of the preview image.
	Width int32 `protobuf:"varint,1,opt,name=width,proto3" json:"width,omitempty"`
	// Height is the height of the preview image.
	Height int32 `protobuf:"varint,2,opt,name=height,proto3" json:"height,omitempty"`
	// URL is the URL of the preview, can be used to preview the file.
	Url string `protobuf:"bytes,3,opt,name=url,proto3" json:"url,omitempty"`
	// Type is the media type of the preview.
	Type          PreviewType `protobuf:"varint,4,opt,name=type,proto3,enum=ttab.ntb.PreviewType" json:"type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Preview) Reset() {
	*x = Preview{}
	mi := &file_ntb_service_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Preview) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Preview) ProtoMessage() {}

func (x *Preview) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Preview.ProtoReflect.Descriptor instead.
func (*Preview) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{15}
}

func (x *Preview) GetWidth() int32 {
	if x != nil {
		return x.Width
	}
	return 0
}

func (x *Preview) GetHeight() int32 {
	if x != nil {
		return x.Height
	}
	return 0
}

func (x *Preview) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

func (x *Preview) GetType() PreviewType {
	if x != nil {
		return x.Type
	}
	return PreviewType_PREVIEW_UNKNOWN
}

type Permissions struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Edit indicates if the user can edit this media.
	Edit bool `protobuf:"varint,1,opt,name=edit,proto3" json:"edit,omitempty"`
	// Delete indicates if the user can delete this media.
	Delete bool `protobuf:"varint,2,opt,name=delete,proto3" json:"delete,omitempty"`
	// Download indicates if the user can download this media.
	Download      bool `protobuf:"varint,3,opt,name=download,proto3" json:"download,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Permissions) Reset() {
	*x = Permissions{}
	mi := &file_ntb_service_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Permissions) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Permissions) ProtoMessage() {}

func (x *Permissions) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Permissions.ProtoReflect.Descriptor instead.
func (*Permissions) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{16}
}

func (x *Permissions) GetEdit() bool {
	if x != nil {
		return x.Edit
	}
	return false
}

func (x *Permissions) GetDelete() bool {
	if x != nil {
		return x.Delete
	}
	return false
}

func (x *Permissions) GetDownload() bool {
	if x != nil {
		return x.Download
	}
	return false
}

type Internal struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Origin contains information about the origin of the media.
	Origin *OriginInfo `protobuf:"bytes,1,opt,name=origin,proto3" json:"origin,omitempty"`
	// ItemID is a non-unique ID of the media provided under upload.
	ItemId        string `protobuf:"bytes,2,opt,name=item_id,json=itemId,proto3" json:"item_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Internal) Reset() {
	*x = Internal{}
	mi := &file_ntb_service_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Internal) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Internal) ProtoMessage() {}

func (x *Internal) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Internal.ProtoReflect.Descriptor instead.
func (*Internal) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{17}
}

func (x *Internal) GetOrigin() *OriginInfo {
	if x != nil {
		return x.Origin
	}
	return nil
}

func (x *Internal) GetItemId() string {
	if x != nil {
		return x.ItemId
	}
	return ""
}

type OriginInfo struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Key is the key of the origin.
	Key string `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`
	// Name is the human readable name of the origin.
	Name          string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OriginInfo) Reset() {
	*x = OriginInfo{}
	mi := &file_ntb_service_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OriginInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OriginInfo) ProtoMessage() {}

func (x *OriginInfo) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OriginInfo.ProtoReflect.Descriptor instead.
func (*OriginInfo) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{18}
}

func (x *OriginInfo) GetKey() string {
	if x != nil {
		return x.Key
	}
	return ""
}

func (x *OriginInfo) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type GetSortFieldsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSortFieldsRequest) Reset() {
	*x = GetSortFieldsRequest{}
	mi := &file_ntb_service_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSortFieldsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSortFieldsRequest) ProtoMessage() {}

func (x *GetSortFieldsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSortFieldsRequest.ProtoReflect.Descriptor instead.
func (*GetSortFieldsRequest) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{19}
}

type GetSortFieldsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Fields        []*SortField           `protobuf:"bytes,1,rep,name=fields,proto3" json:"fields,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSortFieldsResponse) Reset() {
	*x = GetSortFieldsResponse{}
	mi := &file_ntb_service_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSortFieldsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSortFieldsResponse) ProtoMessage() {}

func (x *GetSortFieldsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSortFieldsResponse.ProtoReflect.Descriptor instead.
func (*GetSortFieldsResponse) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{20}
}

func (x *GetSortFieldsResponse) GetFields() []*SortField {
	if x != nil {
		return x.Fields
	}
	return nil
}

type SortField struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SortField) Reset() {
	*x = SortField{}
	mi := &file_ntb_service_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SortField) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SortField) ProtoMessage() {}

func (x *SortField) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SortField.ProtoReflect.Descriptor instead.
func (*SortField) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{21}
}

func (x *SortField) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type StringList struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Values        []string               `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StringList) Reset() {
	*x = StringList{}
	mi := &file_ntb_service_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StringList) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StringList) ProtoMessage() {}

func (x *StringList) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StringList.ProtoReflect.Descriptor instead.
func (*StringList) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{22}
}

func (x *StringList) GetValues() []string {
	if x != nil {
		return x.Values
	}
	return nil
}

type StringMap struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Entries       map[string]string      `protobuf:"bytes,1,rep,name=entries,proto3" json:"entries,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StringMap) Reset() {
	*x = StringMap{}
	mi := &file_ntb_service_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StringMap) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StringMap) ProtoMessage() {}

func (x *StringMap) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StringMap.ProtoReflect.Descriptor instead.
func (*StringMap) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{23}
}

func (x *StringMap) GetEntries() map[string]string {
	if x != nil {
		return x.Entries
	}
	return nil
}

// PreferenceValue represents a language preference setting. It can be a
// boolean to enable/disable a form, or a list of specific words to include or
// exclude. Prefix a word with "-" to exclude it from the form.
type PreferenceValue struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Value:
	//
	//	*PreferenceValue_Enabled
	//	*PreferenceValue_Words
	Value         isPreferenceValue_Value `protobuf_oneof:"value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PreferenceValue) Reset() {
	*x = PreferenceValue{}
	mi := &file_ntb_service_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PreferenceValue) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PreferenceValue) ProtoMessage() {}

func (x *PreferenceValue) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PreferenceValue.ProtoReflect.Descriptor instead.
func (*PreferenceValue) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{24}
}

func (x *PreferenceValue) GetValue() isPreferenceValue_Value {
	if x != nil {
		return x.Value
	}
	return nil
}

func (x *PreferenceValue) GetEnabled() bool {
	if x != nil {
		if x, ok := x.Value.(*PreferenceValue_Enabled); ok {
			return x.Enabled
		}
	}
	return false
}

func (x *PreferenceValue) GetWords() *StringList {
	if x != nil {
		if x, ok := x.Value.(*PreferenceValue_Words); ok {
			return x.Words
		}
	}
	return nil
}

type isPreferenceValue_Value interface {
	isPreferenceValue_Value()
}

type PreferenceValue_Enabled struct {
	Enabled bool `protobuf:"varint,1,opt,name=enabled,proto3,oneof"`
}

type PreferenceValue_Words struct {
	Words *StringList `protobuf:"bytes,2,opt,name=words,proto3,oneof"`
}

func (*PreferenceValue_Enabled) isPreferenceValue_Value() {}

func (*PreferenceValue_Words) isPreferenceValue_Value() {}

type TranslateRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The document content to translate.
	//
	// Types that are valid to be assigned to Document:
	//
	//	*TranslateRequest_Text
	//	*TranslateRequest_Texts
	//	*TranslateRequest_TextMap
	Document isTranslateRequest_Document `protobuf_oneof:"document"`
	// The document format (e.g. "html", "txt", "docx", "json"). Defaults to
	// "html".
	FileType string `protobuf:"bytes,4,opt,name=file_type,json=fileType,proto3" json:"file_type,omitempty"`
	// Controls translation of text inside HTML lang-tagged elements. Only
	// applies when file_type is "html".
	LangParameterTranslate LangParameterTranslate `protobuf:"varint,5,opt,name=lang_parameter_translate,json=langParameterTranslate,proto3,enum=ttab.ntb.LangParameterTranslate" json:"lang_parameter_translate,omitempty"`
	// A preference template name ("standard", "tradisjonell", or "moderne").
	// Can be combined with individual prefs.
	PrefsTemplate string `protobuf:"bytes,6,opt,name=prefs_template,json=prefsTemplate,proto3" json:"prefs_template,omitempty"`
	// Optional GUID to identify this document for translation tracking. If
	// empty, the service generates one.
	Guid string `protobuf:"bytes,7,opt,name=guid,proto3" json:"guid,omitempty"`
	// Source language as an ISO 639-1 code, optionally with a region subtag
	// (e.g. "nb", "nb-no", "nn", "sv", "da"). Defaults to "nb".
	SourceLanguage string `protobuf:"bytes,8,opt,name=source_language,json=sourceLanguage,proto3" json:"source_language,omitempty"`
	// Target language as an ISO 639-1 code, optionally with a region subtag
	// (e.g. "nn", "nb", "sv", "da"). Defaults to "nn".
	TargetLanguage string `protobuf:"bytes,9,opt,name=target_language,json=targetLanguage,proto3" json:"target_language,omitempty"`
	// Enable AI-assisted post-editing of the translation.
	PostEdit bool `protobuf:"varint,10,opt,name=post_edit,json=postEdit,proto3" json:"post_edit,omitempty"`
	// Optional language form preferences keyed by form name.
	Prefs         map[string]*PreferenceValue `protobuf:"bytes,11,rep,name=prefs,proto3" json:"prefs,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TranslateRequest) Reset() {
	*x = TranslateRequest{}
	mi := &file_ntb_service_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TranslateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TranslateRequest) ProtoMessage() {}

func (x *TranslateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TranslateRequest.ProtoReflect.Descriptor instead.
func (*TranslateRequest) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{25}
}

func (x *TranslateRequest) GetDocument() isTranslateRequest_Document {
	if x != nil {
		return x.Document
	}
	return nil
}

func (x *TranslateRequest) GetText() string {
	if x != nil {
		if x, ok := x.Document.(*TranslateRequest_Text); ok {
			return x.Text
		}
	}
	return ""
}

func (x *TranslateRequest) GetTexts() *StringList {
	if x != nil {
		if x, ok := x.Document.(*TranslateRequest_Texts); ok {
			return x.Texts
		}
	}
	return nil
}

func (x *TranslateRequest) GetTextMap() *StringMap {
	if x != nil {
		if x, ok := x.Document.(*TranslateRequest_TextMap); ok {
			return x.TextMap
		}
	}
	return nil
}

func (x *TranslateRequest) GetFileType() string {
	if x != nil {
		return x.FileType
	}
	return ""
}

func (x *TranslateRequest) GetLangParameterTranslate() LangParameterTranslate {
	if x != nil {
		return x.LangParameterTranslate
	}
	return LangParameterTranslate_LANG_PARAMETER_TRANSLATE_UNSPECIFIED
}

func (x *TranslateRequest) GetPrefsTemplate() string {
	if x != nil {
		return x.PrefsTemplate
	}
	return ""
}

func (x *TranslateRequest) GetGuid() string {
	if x != nil {
		return x.Guid
	}
	return ""
}

func (x *TranslateRequest) GetSourceLanguage() string {
	if x != nil {
		return x.SourceLanguage
	}
	return ""
}

func (x *TranslateRequest) GetTargetLanguage() string {
	if x != nil {
		return x.TargetLanguage
	}
	return ""
}

func (x *TranslateRequest) GetPostEdit() bool {
	if x != nil {
		return x.PostEdit
	}
	return false
}

func (x *TranslateRequest) GetPrefs() map[string]*PreferenceValue {
	if x != nil {
		return x.Prefs
	}
	return nil
}

type isTranslateRequest_Document interface {
	isTranslateRequest_Document()
}

type TranslateRequest_Text struct {
	// A single document as plain text or base64-encoded binary.
	Text string `protobuf:"bytes,1,opt,name=text,proto3,oneof"`
}

type TranslateRequest_Texts struct {
	// Multiple document fragments as a list, translated in order.
	Texts *StringList `protobuf:"bytes,2,opt,name=texts,proto3,oneof"`
}

type TranslateRequest_TextMap struct {
	// Multiple document fragments as named key-value pairs.
	TextMap *StringMap `protobuf:"bytes,3,opt,name=text_map,json=textMap,proto3,oneof"`
}

func (*TranslateRequest_Text) isTranslateRequest_Document() {}

func (*TranslateRequest_Texts) isTranslateRequest_Document() {}

func (*TranslateRequest_TextMap) isTranslateRequest_Document() {}

type TranslateResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The translated document in the same format as the request.
	//
	// Types that are valid to be assigned to Document:
	//
	//	*TranslateResponse_Text
	//	*TranslateResponse_Texts
	//	*TranslateResponse_TextMap
	Document isTranslateResponse_Document `protobuf_oneof:"document"`
	// The GUID associated with this translation.
	Guid          string `protobuf:"bytes,4,opt,name=guid,proto3" json:"guid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TranslateResponse) Reset() {
	*x = TranslateResponse{}
	mi := &file_ntb_service_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TranslateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TranslateResponse) ProtoMessage() {}

func (x *TranslateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TranslateResponse.ProtoReflect.Descriptor instead.
func (*TranslateResponse) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{26}
}

func (x *TranslateResponse) GetDocument() isTranslateResponse_Document {
	if x != nil {
		return x.Document
	}
	return nil
}

func (x *TranslateResponse) GetText() string {
	if x != nil {
		if x, ok := x.Document.(*TranslateResponse_Text); ok {
			return x.Text
		}
	}
	return ""
}

func (x *TranslateResponse) GetTexts() *StringList {
	if x != nil {
		if x, ok := x.Document.(*TranslateResponse_Texts); ok {
			return x.Texts
		}
	}
	return nil
}

func (x *TranslateResponse) GetTextMap() *StringMap {
	if x != nil {
		if x, ok := x.Document.(*TranslateResponse_TextMap); ok {
			return x.TextMap
		}
	}
	return nil
}

func (x *TranslateResponse) GetGuid() string {
	if x != nil {
		return x.Guid
	}
	return ""
}

type isTranslateResponse_Document interface {
	isTranslateResponse_Document()
}

type TranslateResponse_Text struct {
	Text string `protobuf:"bytes,1,opt,name=text,proto3,oneof"`
}

type TranslateResponse_Texts struct {
	Texts *StringList `protobuf:"bytes,2,opt,name=texts,proto3,oneof"`
}

type TranslateResponse_TextMap struct {
	TextMap *StringMap `protobuf:"bytes,3,opt,name=text_map,json=textMap,proto3,oneof"`
}

func (*TranslateResponse_Text) isTranslateResponse_Document() {}

func (*TranslateResponse_Texts) isTranslateResponse_Document() {}

func (*TranslateResponse_TextMap) isTranslateResponse_Document() {}

type GetLanguagePrefsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetLanguagePrefsRequest) Reset() {
	*x = GetLanguagePrefsRequest{}
	mi := &file_ntb_service_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetLanguagePrefsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetLanguagePrefsRequest) ProtoMessage() {}

func (x *GetLanguagePrefsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetLanguagePrefsRequest.ProtoReflect.Descriptor instead.
func (*GetLanguagePrefsRequest) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{27}
}

// LanguagePreference describes a single optional language form.
type LanguagePreference struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Category of the language form (e.g. "Konsonantar", "Synonym og stavemåtar").
	Category string `protobuf:"bytes,1,opt,name=category,proto3" json:"category,omitempty"`
	// Human-readable description of what this form changes.
	Description string `protobuf:"bytes,2,opt,name=description,proto3" json:"description,omitempty"`
	// Words that can be individually selected for this form. Empty if the form
	// applies to all words in its category.
	ModifyFor     []string `protobuf:"bytes,3,rep,name=modify_for,json=modifyFor,proto3" json:"modify_for,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LanguagePreference) Reset() {
	*x = LanguagePreference{}
	mi := &file_ntb_service_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LanguagePreference) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LanguagePreference) ProtoMessage() {}

func (x *LanguagePreference) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LanguagePreference.ProtoReflect.Descriptor instead.
func (*LanguagePreference) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{28}
}

func (x *LanguagePreference) GetCategory() string {
	if x != nil {
		return x.Category
	}
	return ""
}

func (x *LanguagePreference) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *LanguagePreference) GetModifyFor() []string {
	if x != nil {
		return x.ModifyFor
	}
	return nil
}

type GetLanguagePrefsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Map of form name to its details.
	LanguagePrefs map[string]*LanguagePreference `protobuf:"bytes,1,rep,name=language_prefs,json=languagePrefs,proto3" json:"language_prefs,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetLanguagePrefsResponse) Reset() {
	*x = GetLanguagePrefsResponse{}
	mi := &file_ntb_service_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetLanguagePrefsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetLanguagePrefsResponse) ProtoMessage() {}

func (x *GetLanguagePrefsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetLanguagePrefsResponse.ProtoReflect.Descriptor instead.
func (*GetLanguagePrefsResponse) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{29}
}

func (x *GetLanguagePrefsResponse) GetLanguagePrefs() map[string]*LanguagePreference {
	if x != nil {
		return x.LanguagePrefs
	}
	return nil
}

type GetDocumentFormatsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDocumentFormatsRequest) Reset() {
	*x = GetDocumentFormatsRequest{}
	mi := &file_ntb_service_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentFormatsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentFormatsRequest) ProtoMessage() {}

func (x *GetDocumentFormatsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentFormatsRequest.ProtoReflect.Descriptor instead.
func (*GetDocumentFormatsRequest) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{30}
}

// DocumentFormatInfo describes a supported document format.
type DocumentFormatInfo struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The file type identifier used in translation requests (e.g. "html", "docx").
	FileType string `protobuf:"bytes,1,opt,name=file_type,json=fileType,proto3" json:"file_type,omitempty"`
	// File suffixes associated with this format (e.g. [".html", ".htm"]).
	FileSuffixes []string `protobuf:"bytes,2,rep,name=file_suffixes,json=fileSuffixes,proto3" json:"file_suffixes,omitempty"`
	// MIME types associated with this format (e.g. ["text/html"]).
	MimeTypes []string `protobuf:"bytes,3,rep,name=mime_types,json=mimeTypes,proto3" json:"mime_types,omitempty"`
	// Whether document content must be base64-encoded for this format.
	Base64Encode  bool `protobuf:"varint,4,opt,name=base64_encode,json=base64Encode,proto3" json:"base64_encode,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DocumentFormatInfo) Reset() {
	*x = DocumentFormatInfo{}
	mi := &file_ntb_service_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DocumentFormatInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DocumentFormatInfo) ProtoMessage() {}

func (x *DocumentFormatInfo) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DocumentFormatInfo.ProtoReflect.Descriptor instead.
func (*DocumentFormatInfo) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{31}
}

func (x *DocumentFormatInfo) GetFileType() string {
	if x != nil {
		return x.FileType
	}
	return ""
}

func (x *DocumentFormatInfo) GetFileSuffixes() []string {
	if x != nil {
		return x.FileSuffixes
	}
	return nil
}

func (x *DocumentFormatInfo) GetMimeTypes() []string {
	if x != nil {
		return x.MimeTypes
	}
	return nil
}

func (x *DocumentFormatInfo) GetBase64Encode() bool {
	if x != nil {
		return x.Base64Encode
	}
	return false
}

type GetDocumentFormatsResponse struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	DocumentFormats []*DocumentFormatInfo  `protobuf:"bytes,1,rep,name=document_formats,json=documentFormats,proto3" json:"document_formats,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *GetDocumentFormatsResponse) Reset() {
	*x = GetDocumentFormatsResponse{}
	mi := &file_ntb_service_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentFormatsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentFormatsResponse) ProtoMessage() {}

func (x *GetDocumentFormatsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentFormatsResponse.ProtoReflect.Descriptor instead.
func (*GetDocumentFormatsResponse) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{32}
}

func (x *GetDocumentFormatsResponse) GetDocumentFormats() []*DocumentFormatInfo {
	if x != nil {
		return x.DocumentFormats
	}
	return nil
}

type GetPreferenceTemplatesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetPreferenceTemplatesRequest) Reset() {
	*x = GetPreferenceTemplatesRequest{}
	mi := &file_ntb_service_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetPreferenceTemplatesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetPreferenceTemplatesRequest) ProtoMessage() {}

func (x *GetPreferenceTemplatesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetPreferenceTemplatesRequest.ProtoReflect.Descriptor instead.
func (*GetPreferenceTemplatesRequest) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{33}
}

type GetPreferenceTemplatesResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Map of template name to the list of language form names it enables.
	PreferenceTemplates map[string]*StringList `protobuf:"bytes,1,rep,name=preference_templates,json=preferenceTemplates,proto3" json:"preference_templates,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *GetPreferenceTemplatesResponse) Reset() {
	*x = GetPreferenceTemplatesResponse{}
	mi := &file_ntb_service_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetPreferenceTemplatesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetPreferenceTemplatesResponse) ProtoMessage() {}

func (x *GetPreferenceTemplatesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_ntb_service_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetPreferenceTemplatesResponse.ProtoReflect.Descriptor instead.
func (*GetPreferenceTemplatesResponse) Descriptor() ([]byte, []int) {
	return file_ntb_service_proto_rawDescGZIP(), []int{34}
}

func (x *GetPreferenceTemplatesResponse) GetPreferenceTemplates() map[string]*StringList {
	if x != nil {
		return x.PreferenceTemplates
	}
	return nil
}

var File_ntb_service_proto protoreflect.FileDescriptor

const file_ntb_service_proto_rawDesc = "" +
	"\n" +
	"\x11ntb/service.proto\x12\bttab.ntb\"?\n" +
	"\x14GetAssignmentRequest\x12'\n" +
	"\x0fcommission_code\x18\x01 \x01(\tR\x0ecommissionCode\"e\n" +
	"\x15GetAssignmentResponse\x12#\n" +
	"\rplanning_uuid\x18\x01 \x01(\tR\fplanningUuid\x12'\n" +
	"\x0fassignment_uuid\x18\x02 \x01(\tR\x0eassignmentUuid\"\x18\n" +
	"\x16ListAssignmentsRequest\"\xcd\x01\n" +
	"\n" +
	"Assignment\x12#\n" +
	"\rplanning_uuid\x18\x01 \x01(\tR\fplanningUuid\x12'\n" +
	"\x0fassignment_uuid\x18\x02 \x01(\tR\x0eassignmentUuid\x12#\n" +
	"\rassignee_uuid\x18\x03 \x01(\tR\fassigneeUuid\x12#\n" +
	"\rassignee_name\x18\x04 \x01(\tR\fassigneeName\x12'\n" +
	"\x0fcommission_code\x18\x05 \x01(\tR\x0ecommissionCode\"Q\n" +
	"\x17ListAssignmentsResponse\x126\n" +
	"\vassignments\x18\x01 \x03(\v2\x14.ttab.ntb.AssignmentR\vassignments\":\n" +
	"\x0eGetItemRequest\x12\x18\n" +
	"\aarchive\x18\x01 \x01(\tR\aarchive\x12\x0e\n" +
	"\x02id\x18\x02 \x01(\tR\x02id\":\n" +
	"\x0fGetItemResponse\x12'\n" +
	"\x04item\x18\x01 \x01(\v2\x13.ttab.ntb.MediaItemR\x04item\"\x8c\a\n" +
	"\rSearchRequest\x12\x18\n" +
	"\aarchive\x18\x01 \x01(\tR\aarchive\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x16\n" +
	"\x06offset\x18\x03 \x01(\x05R\x06offset\x12\"\n" +
	"\x04sort\x18\x04 \x01(\v2\x0e.ttab.ntb.SortR\x04sort\x12\x14\n" +
	"\x05query\x18\x05 \x01(\tR\x05query\x12\x17\n" +
	"\aitem_id\x18\x06 \x01(\tR\x06itemId\x12\x12\n" +
	"\x04city\x18\a \x01(\tR\x04city\x12\x18\n" +
	"\acountry\x18\b \x01(\tR\acountry\x12\x18\n" +
	"\acreator\x18\t \x01(\tR\acreator\x12\x10\n" +
	"\x03ids\x18\n" +
	" \x03(\tR\x03ids\x12\x1a\n" +
	"\bkeywords\x18\v \x03(\tR\bkeywords\x12 \n" +
	"\vcollections\x18\f \x03(\x05R\vcollections\x12\x16\n" +
	"\x06albums\x18\r \x03(\x05R\x06albums\x12\x1e\n" +
	"\n" +
	"categories\x18\x0e \x03(\tR\n" +
	"categories\x12\x1c\n" +
	"\tlocations\x18\x0f \x01(\tR\tlocations\x12\x15\n" +
	"\x06job_id\x18\x10 \x01(\tR\x05jobId\x127\n" +
	"\vorientation\x18\x11 \x01(\x0e2\x15.ttab.ntb.OrientationR\vorientation\x12!\n" +
	"\fsearch_after\x18\x12 \x03(\tR\vsearchAfter\x12*\n" +
	"\aorigins\x18\x13 \x03(\x0e2\x10.ttab.ntb.OriginR\aorigins\x12/\n" +
	"\tfile_type\x18\x14 \x01(\x0e2\x12.ttab.ntb.FileTypeR\bfileType\x12\x1f\n" +
	"\vcreated_min\x18\x15 \x01(\tR\n" +
	"createdMin\x12\x1f\n" +
	"\vcreated_max\x18\x16 \x01(\tR\n" +
	"createdMax\x121\n" +
	"\x14outside_subscription\x18\x17 \x01(\bR\x13outsideSubscription\x12)\n" +
	"\x10editorial_topics\x18\x18 \x03(\tR\x0feditorialTopics\x12+\n" +
	"\x11distributor_names\x18\x19 \x03(\tR\x10distributorNames\x12\x16\n" +
	"\x06events\x18\x1a \x03(\tR\x06events\x12#\n" +
	"\rsubject_terms\x18\x1b \x03(\tR\fsubjectTerms\x12\x18\n" +
	"\apersons\x18\x1c \x03(\tR\apersons\"<\n" +
	"\x04Sort\x12\x14\n" +
	"\x05field\x18\x01 \x01(\tR\x05field\x12\x1e\n" +
	"\n" +
	"descending\x18\x02 \x01(\bR\n" +
	"descending\"]\n" +
	"\x0eSearchResponse\x12\"\n" +
	"\x04page\x18\x01 \x01(\v2\x0e.ttab.ntb.PageR\x04page\x12'\n" +
	"\x04data\x18\x03 \x03(\v2\x13.ttab.ntb.MediaItemR\x04data\"\x83\x01\n" +
	"\x04Page\x12\x16\n" +
	"\x06offset\x18\x01 \x01(\x05R\x06offset\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05count\x18\x03 \x01(\x05R\x05count\x12\x14\n" +
	"\x05total\x18\x04 \x01(\x05R\x05total\x12!\n" +
	"\fsearch_after\x18\x05 \x03(\tR\vsearchAfter\"\xc2\t\n" +
	"\tMediaItem\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x12\n" +
	"\x04type\x18\x02 \x01(\tR\x04type\x12\x18\n" +
	"\aarchive\x18\x03 \x01(\tR\aarchive\x12\x17\n" +
	"\auser_id\x18\x04 \x01(\tR\x06userId\x12\"\n" +
	"\frestrictions\x18\x05 \x01(\bR\frestrictions\x12\x1a\n" +
	"\bheadline\x18\x06 \x01(\tR\bheadline\x12 \n" +
	"\vdescription\x18\a \x01(\tR\vdescription\x124\n" +
	"\x16alt_text_accessibility\x18\b \x01(\tR\x14altTextAccessibility\x12\x18\n" +
	"\acreator\x18\t \x01(\tR\acreator\x12\x1a\n" +
	"\bkeywords\x18\n" +
	" \x03(\tR\bkeywords\x12\x1a\n" +
	"\bcategory\x18\v \x01(\tR\bcategory\x125\n" +
	"\rpersons_shown\x18\f \x03(\v2\x10.ttab.ntb.PersonR\fpersonsShown\x12&\n" +
	"\fimage_rating\x18\r \x01(\x05H\x00R\vimageRating\x88\x01\x01\x12\x14\n" +
	"\x05title\x18\x0e \x01(\tR\x05title\x12#\n" +
	"\rdate_archived\x18\x0f \x01(\tR\fdateArchived\x12?\n" +
	"\fdate_created\x18\x10 \x01(\v2\x1c.ttab.ntb.ImpreciseTimestampR\vdateCreated\x12\x18\n" +
	"\acountry\x18\x11 \x01(\tR\acountry\x12\x12\n" +
	"\x04city\x18\x12 \x01(\tR\x04city\x12\x1f\n" +
	"\vcredit_line\x18\x13 \x01(\tR\n" +
	"creditLine\x12#\n" +
	"\rversion_count\x18\x14 \x01(\x05R\fversionCount\x12\"\n" +
	"\finstructions\x18\x15 \x01(\tR\finstructions\x12\x1f\n" +
	"\vusage_terms\x18\x16 \x01(\tR\n" +
	"usageTerms\x12\x16\n" +
	"\x06source\x18\x17 \x01(\tR\x06source\x12\x1c\n" +
	"\tsuppliers\x18\x18 \x01(\tR\tsuppliers\x125\n" +
	"\x17image_supplier_image_id\x18\x19 \x01(\tR\x14imageSupplierImageId\x12\x15\n" +
	"\x06job_id\x18\x1a \x01(\tR\x05jobId\x12\"\n" +
	"\x04file\x18\x1b \x01(\v2\x0e.ttab.ntb.FileR\x04file\x12-\n" +
	"\bpreviews\x18\x1c \x03(\v2\x11.ttab.ntb.PreviewR\bpreviews\x127\n" +
	"\vpermissions\x18\x1d \x01(\v2\x15.ttab.ntb.PermissionsR\vpermissions\x124\n" +
	"\x05links\x18\x1e \x03(\v2\x1e.ttab.ntb.MediaItem.LinksEntryR\x05links\x12.\n" +
	"\binternal\x18\x1f \x01(\v2\x12.ttab.ntb.InternalR\binternal\x12\x1f\n" +
	"\vsort_values\x18  \x03(\tR\n" +
	"sortValues\x1a8\n" +
	"\n" +
	"LinksEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01B\x0f\n" +
	"\r_image_rating\"\x8b\x01\n" +
	"\x12ImpreciseTimestamp\x12\x12\n" +
	"\x04time\x18\x01 \x01(\tR\x04time\x12%\n" +
	"\x0evalid_timezone\x18\x02 \x01(\bR\rvalidTimezone\x12:\n" +
	"\tprecision\x18\x03 \x01(\x0e2\x1c.ttab.ntb.TimestampPrecisionR\tprecision\"X\n" +
	"\x06Person\x12\x1f\n" +
	"\vperson_name\x18\x01 \x01(\tR\n" +
	"personName\x12-\n" +
	"\x12person_description\x18\x02 \x01(\tR\x11personDescription\"\x92\x01\n" +
	"\x04File\x12\x12\n" +
	"\x04size\x18\x01 \x01(\x03R\x04size\x12\x16\n" +
	"\x06height\x18\x02 \x01(\x05R\x06height\x12\x14\n" +
	"\x05width\x18\x03 \x01(\x05R\x05width\x12\x1b\n" +
	"\tmime_type\x18\x04 \x01(\tR\bmimeType\x12+\n" +
	"\x11original_filename\x18\x05 \x01(\tR\x10originalFilename\"t\n" +
	"\aPreview\x12\x14\n" +
	"\x05width\x18\x01 \x01(\x05R\x05width\x12\x16\n" +
	"\x06height\x18\x02 \x01(\x05R\x06height\x12\x10\n" +
	"\x03url\x18\x03 \x01(\tR\x03url\x12)\n" +
	"\x04type\x18\x04 \x01(\x0e2\x15.ttab.ntb.PreviewTypeR\x04type\"U\n" +
	"\vPermissions\x12\x12\n" +
	"\x04edit\x18\x01 \x01(\bR\x04edit\x12\x16\n" +
	"\x06delete\x18\x02 \x01(\bR\x06delete\x12\x1a\n" +
	"\bdownload\x18\x03 \x01(\bR\bdownload\"Q\n" +
	"\bInternal\x12,\n" +
	"\x06origin\x18\x01 \x01(\v2\x14.ttab.ntb.OriginInfoR\x06origin\x12\x17\n" +
	"\aitem_id\x18\x02 \x01(\tR\x06itemId\"2\n" +
	"\n" +
	"OriginInfo\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\"\x16\n" +
	"\x14GetSortFieldsRequest\"D\n" +
	"\x15GetSortFieldsResponse\x12+\n" +
	"\x06fields\x18\x01 \x03(\v2\x13.ttab.ntb.SortFieldR\x06fields\"\x1f\n" +
	"\tSortField\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\"$\n" +
	"\n" +
	"StringList\x12\x16\n" +
	"\x06values\x18\x01 \x03(\tR\x06values\"\x83\x01\n" +
	"\tStringMap\x12:\n" +
	"\aentries\x18\x01 \x03(\v2 .ttab.ntb.StringMap.EntriesEntryR\aentries\x1a:\n" +
	"\fEntriesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"d\n" +
	"\x0fPreferenceValue\x12\x1a\n" +
	"\aenabled\x18\x01 \x01(\bH\x00R\aenabled\x12,\n" +
	"\x05words\x18\x02 \x01(\v2\x14.ttab.ntb.StringListH\x00R\x05wordsB\a\n" +
	"\x05value\"\xc9\x04\n" +
	"\x10TranslateRequest\x12\x14\n" +
	"\x04text\x18\x01 \x01(\tH\x00R\x04text\x12,\n" +
	"\x05texts\x18\x02 \x01(\v2\x14.ttab.ntb.StringListH\x00R\x05texts\x120\n" +
	"\btext_map\x18\x03 \x01(\v2\x13.ttab.ntb.StringMapH\x00R\atextMap\x12\x1b\n" +
	"\tfile_type\x18\x04 \x01(\tR\bfileType\x12Z\n" +
	"\x18lang_parameter_translate\x18\x05 \x01(\x0e2 .ttab.ntb.LangParameterTranslateR\x16langParameterTranslate\x12%\n" +
	"\x0eprefs_template\x18\x06 \x01(\tR\rprefsTemplate\x12\x12\n" +
	"\x04guid\x18\a \x01(\tR\x04guid\x12'\n" +
	"\x0fsource_language\x18\b \x01(\tR\x0esourceLanguage\x12'\n" +
	"\x0ftarget_language\x18\t \x01(\tR\x0etargetLanguage\x12\x1b\n" +
	"\tpost_edit\x18\n" +
	" \x01(\bR\bpostEdit\x12;\n" +
	"\x05prefs\x18\v \x03(\v2%.ttab.ntb.TranslateRequest.PrefsEntryR\x05prefs\x1aS\n" +
	"\n" +
	"PrefsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12/\n" +
	"\x05value\x18\x02 \x01(\v2\x19.ttab.ntb.PreferenceValueR\x05value:\x028\x01B\n" +
	"\n" +
	"\bdocument\"\xa9\x01\n" +
	"\x11TranslateResponse\x12\x14\n" +
	"\x04text\x18\x01 \x01(\tH\x00R\x04text\x12,\n" +
	"\x05texts\x18\x02 \x01(\v2\x14.ttab.ntb.StringListH\x00R\x05texts\x120\n" +
	"\btext_map\x18\x03 \x01(\v2\x13.ttab.ntb.StringMapH\x00R\atextMap\x12\x12\n" +
	"\x04guid\x18\x04 \x01(\tR\x04guidB\n" +
	"\n" +
	"\bdocument\"\x19\n" +
	"\x17GetLanguagePrefsRequest\"q\n" +
	"\x12LanguagePreference\x12\x1a\n" +
	"\bcategory\x18\x01 \x01(\tR\bcategory\x12 \n" +
	"\vdescription\x18\x02 \x01(\tR\vdescription\x12\x1d\n" +
	"\n" +
	"modify_for\x18\x03 \x03(\tR\tmodifyFor\"\xd8\x01\n" +
	"\x18GetLanguagePrefsResponse\x12\\\n" +
	"\x0elanguage_prefs\x18\x01 \x03(\v25.ttab.ntb.GetLanguagePrefsResponse.LanguagePrefsEntryR\rlanguagePrefs\x1a^\n" +
	"\x12LanguagePrefsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x122\n" +
	"\x05value\x18\x02 \x01(\v2\x1c.ttab.ntb.LanguagePreferenceR\x05value:\x028\x01\"\x1b\n" +
	"\x19GetDocumentFormatsRequest\"\x9a\x01\n" +
	"\x12DocumentFormatInfo\x12\x1b\n" +
	"\tfile_type\x18\x01 \x01(\tR\bfileType\x12#\n" +
	"\rfile_suffixes\x18\x02 \x03(\tR\ffileSuffixes\x12\x1d\n" +
	"\n" +
	"mime_types\x18\x03 \x03(\tR\tmimeTypes\x12#\n" +
	"\rbase64_encode\x18\x04 \x01(\bR\fbase64Encode\"e\n" +
	"\x1aGetDocumentFormatsResponse\x12G\n" +
	"\x10document_formats\x18\x01 \x03(\v2\x1c.ttab.ntb.DocumentFormatInfoR\x0fdocumentFormats\"\x1f\n" +
	"\x1dGetPreferenceTemplatesRequest\"\xf4\x01\n" +
	"\x1eGetPreferenceTemplatesResponse\x12t\n" +
	"\x14preference_templates\x18\x01 \x03(\v2A.ttab.ntb.GetPreferenceTemplatesResponse.PreferenceTemplatesEntryR\x13preferenceTemplates\x1a\\\n" +
	"\x18PreferenceTemplatesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12*\n" +
	"\x05value\x18\x02 \x01(\v2\x14.ttab.ntb.StringListR\x05value:\x028\x01*x\n" +
	"\vOrientation\x12\x1b\n" +
	"\x17ORIENTATION_UNSPECIFIED\x10\x00\x12\x1a\n" +
	"\x16ORIENTATION_HORIZONTAL\x10\x01\x12\x18\n" +
	"\x14ORIENTATION_VERTICAL\x10\x02\x12\x16\n" +
	"\x12ORIENTATION_SQUARE\x10\x03*\xb8\x01\n" +
	"\bLocation\x12\x18\n" +
	"\x14LOCATION_UNSPECIFIED\x10\x00\x12\x17\n" +
	"\x13LOCATION_IN_GALLERY\x10\x01\x12\x1b\n" +
	"\x17LOCATION_NOT_IN_GALLERY\x10\x02\x12\x1b\n" +
	"\x17LOCATION_IN_COLLECTIONS\x10\x03\x12\x1f\n" +
	"\x1bLOCATION_NOT_IN_COLLECTIONS\x10\x04\x12\x1e\n" +
	"\x1aLOCATION_ALL_ORGANIZATIONS\x10\x05*\xbe\x01\n" +
	"\x06Origin\x12\x16\n" +
	"\x12ORIGIN_UNSPECIFIED\x10\x00\x12\x11\n" +
	"\rORIGIN_UPLOAD\x10\x01\x12\x10\n" +
	"\fORIGIN_GUEST\x10\x02\x12\x13\n" +
	"\x0fORIGIN_GUESTFTP\x10\x03\x12\x10\n" +
	"\fORIGIN_EMAIL\x10\x04\x12\x11\n" +
	"\rORIGIN_IMPORT\x10\x05\x12\x0e\n" +
	"\n" +
	"ORIGIN_FTP\x10\x06\x12\x0e\n" +
	"\n" +
	"ORIGIN_LTR\x10\a\x12\r\n" +
	"\tORIGIN_PS\x10\b\x12\x0e\n" +
	"\n" +
	"ORIGIN_CMS\x10\t*\x91\x01\n" +
	"\bFileType\x12\x19\n" +
	"\x15FILE_TYPE_UNSPECIFIED\x10\x00\x12\x13\n" +
	"\x0fFILE_TYPE_IMAGE\x10\x01\x12\x13\n" +
	"\x0fFILE_TYPE_VIDEO\x10\x02\x12\x11\n" +
	"\rFILE_TYPE_PDF\x10\x03\x12\x14\n" +
	"\x10FILE_TYPE_VECTOR\x10\x04\x12\x17\n" +
	"\x13FILE_TYPE_PHOTOSHOP\x10\x05*\xed\x01\n" +
	"\x12TimestampPrecision\x12\x18\n" +
	"\x14TS_PRECISION_UNKNOWN\x10\x00\x12\x15\n" +
	"\x11TS_PRECISION_FULL\x10\x01\x12\x17\n" +
	"\x13TS_PRECISION_MINUTE\x10\x02\x12\x15\n" +
	"\x11TS_PRECISION_HOUR\x10\x03\x12\x14\n" +
	"\x10TS_PRECISION_DAY\x10\x04\x12\x16\n" +
	"\x12TS_PRECISION_MONTH\x10\x05\x12\x15\n" +
	"\x11TS_PRECISION_YEAR\x10\x06\x12\x17\n" +
	"\x13TS_PRECISION_DECADE\x10\a\x12\x18\n" +
	"\x14TS_PRECISION_CENTURY\x10\b*H\n" +
	"\vPreviewType\x12\x13\n" +
	"\x0fPREVIEW_UNKNOWN\x10\x00\x12\x11\n" +
	"\rPREVIEW_IMAGE\x10\x01\x12\x11\n" +
	"\rPREVIEW_VIDEO\x10\x02*\xac\x01\n" +
	"\x16LangParameterTranslate\x12(\n" +
	"$LANG_PARAMETER_TRANSLATE_UNSPECIFIED\x10\x00\x12#\n" +
	"\x1fLANG_PARAMETER_TRANSLATE_ORIGIN\x10\x01\x12 \n" +
	"\x1cLANG_PARAMETER_TRANSLATE_ALL\x10\x02\x12!\n" +
	"\x1dLANG_PARAMETER_TRANSLATE_NONE\x10\x032\xb4\x01\n" +
	"\bMetadata\x12P\n" +
	"\rGetAssignment\x12\x1e.ttab.ntb.GetAssignmentRequest\x1a\x1f.ttab.ntb.GetAssignmentResponse\x12V\n" +
	"\x0fListAssignments\x12 .ttab.ntb.ListAssignmentsRequest\x1a!.ttab.ntb.ListAssignmentsResponse2\xd6\x01\n" +
	"\x05Media\x12;\n" +
	"\x06Search\x12\x17.ttab.ntb.SearchRequest\x1a\x18.ttab.ntb.SearchResponse\x12>\n" +
	"\aGetItem\x12\x18.ttab.ntb.GetItemRequest\x1a\x19.ttab.ntb.GetItemResponse\x12P\n" +
	"\rGetSortFields\x12\x1e.ttab.ntb.GetSortFieldsRequest\x1a\x1f.ttab.ntb.GetSortFieldsResponse2\xf8\x02\n" +
	"\aNynorsk\x12D\n" +
	"\tTranslate\x12\x1a.ttab.ntb.TranslateRequest\x1a\x1b.ttab.ntb.TranslateResponse\x12Y\n" +
	"\x10GetLanguagePrefs\x12!.ttab.ntb.GetLanguagePrefsRequest\x1a\".ttab.ntb.GetLanguagePrefsResponse\x12_\n" +
	"\x12GetDocumentFormats\x12#.ttab.ntb.GetDocumentFormatsRequest\x1a$.ttab.ntb.GetDocumentFormatsResponse\x12k\n" +
	"\x16GetPreferenceTemplates\x12'.ttab.ntb.GetPreferenceTemplatesRequest\x1a(.ttab.ntb.GetPreferenceTemplatesResponseB%Z#github.com/ttab/elephant-tt-api/ntbb\x06proto3"

var (
	file_ntb_service_proto_rawDescOnce sync.Once
	file_ntb_service_proto_rawDescData []byte
)

func file_ntb_service_proto_rawDescGZIP() []byte {
	file_ntb_service_proto_rawDescOnce.Do(func() {
		file_ntb_service_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_ntb_service_proto_rawDesc), len(file_ntb_service_proto_rawDesc)))
	})
	return file_ntb_service_proto_rawDescData
}

var file_ntb_service_proto_enumTypes = make([]protoimpl.EnumInfo, 7)
var file_ntb_service_proto_msgTypes = make([]protoimpl.MessageInfo, 40)
var file_ntb_service_proto_goTypes = []any{
	(Orientation)(0),                       // 0: ttab.ntb.Orientation
	(Location)(0),                          // 1: ttab.ntb.Location
	(Origin)(0),                            // 2: ttab.ntb.Origin
	(FileType)(0),                          // 3: ttab.ntb.FileType
	(TimestampPrecision)(0),                // 4: ttab.ntb.TimestampPrecision
	(PreviewType)(0),                       // 5: ttab.ntb.PreviewType
	(LangParameterTranslate)(0),            // 6: ttab.ntb.LangParameterTranslate
	(*GetAssignmentRequest)(nil),           // 7: ttab.ntb.GetAssignmentRequest
	(*GetAssignmentResponse)(nil),          // 8: ttab.ntb.GetAssignmentResponse
	(*ListAssignmentsRequest)(nil),         // 9: ttab.ntb.ListAssignmentsRequest
	(*Assignment)(nil),                     // 10: ttab.ntb.Assignment
	(*ListAssignmentsResponse)(nil),        // 11: ttab.ntb.ListAssignmentsResponse
	(*GetItemRequest)(nil),                 // 12: ttab.ntb.GetItemRequest
	(*GetItemResponse)(nil),                // 13: ttab.ntb.GetItemResponse
	(*SearchRequest)(nil),                  // 14: ttab.ntb.SearchRequest
	(*Sort)(nil),                           // 15: ttab.ntb.Sort
	(*SearchResponse)(nil),                 // 16: ttab.ntb.SearchResponse
	(*Page)(nil),                           // 17: ttab.ntb.Page
	(*MediaItem)(nil),                      // 18: ttab.ntb.MediaItem
	(*ImpreciseTimestamp)(nil),             // 19: ttab.ntb.ImpreciseTimestamp
	(*Person)(nil),                         // 20: ttab.ntb.Person
	(*File)(nil),                           // 21: ttab.ntb.File
	(*Preview)(nil),                        // 22: ttab.ntb.Preview
	(*Permissions)(nil),                    // 23: ttab.ntb.Permissions
	(*Internal)(nil),                       // 24: ttab.ntb.Internal
	(*OriginInfo)(nil),                     // 25: ttab.ntb.OriginInfo
	(*GetSortFieldsRequest)(nil),           // 26: ttab.ntb.GetSortFieldsRequest
	(*GetSortFieldsResponse)(nil),          // 27: ttab.ntb.GetSortFieldsResponse
	(*SortField)(nil),                      // 28: ttab.ntb.SortField
	(*StringList)(nil),                     // 29: ttab.ntb.StringList
	(*StringMap)(nil),                      // 30: ttab.ntb.StringMap
	(*PreferenceValue)(nil),                // 31: ttab.ntb.PreferenceValue
	(*TranslateRequest)(nil),               // 32: ttab.ntb.TranslateRequest
	(*TranslateResponse)(nil),              // 33: ttab.ntb.TranslateResponse
	(*GetLanguagePrefsRequest)(nil),        // 34: ttab.ntb.GetLanguagePrefsRequest
	(*LanguagePreference)(nil),             // 35: ttab.ntb.LanguagePreference
	(*GetLanguagePrefsResponse)(nil),       // 36: ttab.ntb.GetLanguagePrefsResponse
	(*GetDocumentFormatsRequest)(nil),      // 37: ttab.ntb.GetDocumentFormatsRequest
	(*DocumentFormatInfo)(nil),             // 38: ttab.ntb.DocumentFormatInfo
	(*GetDocumentFormatsResponse)(nil),     // 39: ttab.ntb.GetDocumentFormatsResponse
	(*GetPreferenceTemplatesRequest)(nil),  // 40: ttab.ntb.GetPreferenceTemplatesRequest
	(*GetPreferenceTemplatesResponse)(nil), // 41: ttab.ntb.GetPreferenceTemplatesResponse
	nil,                                    // 42: ttab.ntb.MediaItem.LinksEntry
	nil,                                    // 43: ttab.ntb.StringMap.EntriesEntry
	nil,                                    // 44: ttab.ntb.TranslateRequest.PrefsEntry
	nil,                                    // 45: ttab.ntb.GetLanguagePrefsResponse.LanguagePrefsEntry
	nil,                                    // 46: ttab.ntb.GetPreferenceTemplatesResponse.PreferenceTemplatesEntry
}
var file_ntb_service_proto_depIdxs = []int32{
	10, // 0: ttab.ntb.ListAssignmentsResponse.assignments:type_name -> ttab.ntb.Assignment
	18, // 1: ttab.ntb.GetItemResponse.item:type_name -> ttab.ntb.MediaItem
	15, // 2: ttab.ntb.SearchRequest.sort:type_name -> ttab.ntb.Sort
	0,  // 3: ttab.ntb.SearchRequest.orientation:type_name -> ttab.ntb.Orientation
	2,  // 4: ttab.ntb.SearchRequest.origins:type_name -> ttab.ntb.Origin
	3,  // 5: ttab.ntb.SearchRequest.file_type:type_name -> ttab.ntb.FileType
	17, // 6: ttab.ntb.SearchResponse.page:type_name -> ttab.ntb.Page
	18, // 7: ttab.ntb.SearchResponse.data:type_name -> ttab.ntb.MediaItem
	20, // 8: ttab.ntb.MediaItem.persons_shown:type_name -> ttab.ntb.Person
	19, // 9: ttab.ntb.MediaItem.date_created:type_name -> ttab.ntb.ImpreciseTimestamp
	21, // 10: ttab.ntb.MediaItem.file:type_name -> ttab.ntb.File
	22, // 11: ttab.ntb.MediaItem.previews:type_name -> ttab.ntb.Preview
	23, // 12: ttab.ntb.MediaItem.permissions:type_name -> ttab.ntb.Permissions
	42, // 13: ttab.ntb.MediaItem.links:type_name -> ttab.ntb.MediaItem.LinksEntry
	24, // 14: ttab.ntb.MediaItem.internal:type_name -> ttab.ntb.Internal
	4,  // 15: ttab.ntb.ImpreciseTimestamp.precision:type_name -> ttab.ntb.TimestampPrecision
	5,  // 16: ttab.ntb.Preview.type:type_name -> ttab.ntb.PreviewType
	25, // 17: ttab.ntb.Internal.origin:type_name -> ttab.ntb.OriginInfo
	28, // 18: ttab.ntb.GetSortFieldsResponse.fields:type_name -> ttab.ntb.SortField
	43, // 19: ttab.ntb.StringMap.entries:type_name -> ttab.ntb.StringMap.EntriesEntry
	29, // 20: ttab.ntb.PreferenceValue.words:type_name -> ttab.ntb.StringList
	29, // 21: ttab.ntb.TranslateRequest.texts:type_name -> ttab.ntb.StringList
	30, // 22: ttab.ntb.TranslateRequest.text_map:type_name -> ttab.ntb.StringMap
	6,  // 23: ttab.ntb.TranslateRequest.lang_parameter_translate:type_name -> ttab.ntb.LangParameterTranslate
	44, // 24: ttab.ntb.TranslateRequest.prefs:type_name -> ttab.ntb.TranslateRequest.PrefsEntry
	29, // 25: ttab.ntb.TranslateResponse.texts:type_name -> ttab.ntb.StringList
	30, // 26: ttab.ntb.TranslateResponse.text_map:type_name -> ttab.ntb.StringMap
	45, // 27: ttab.ntb.GetLanguagePrefsResponse.language_prefs:type_name -> ttab.ntb.GetLanguagePrefsResponse.LanguagePrefsEntry
	38, // 28: ttab.ntb.GetDocumentFormatsResponse.document_formats:type_name -> ttab.ntb.DocumentFormatInfo
	46, // 29: ttab.ntb.GetPreferenceTemplatesResponse.preference_templates:type_name -> ttab.ntb.GetPreferenceTemplatesResponse.PreferenceTemplatesEntry
	31, // 30: ttab.ntb.TranslateRequest.PrefsEntry.value:type_name -> ttab.ntb.PreferenceValue
	35, // 31: ttab.ntb.GetLanguagePrefsResponse.LanguagePrefsEntry.value:type_name -> ttab.ntb.LanguagePreference
	29, // 32: ttab.ntb.GetPreferenceTemplatesResponse.PreferenceTemplatesEntry.value:type_name -> ttab.ntb.StringList
	7,  // 33: ttab.ntb.Metadata.GetAssignment:input_type -> ttab.ntb.GetAssignmentRequest
	9,  // 34: ttab.ntb.Metadata.ListAssignments:input_type -> ttab.ntb.ListAssignmentsRequest
	14, // 35: ttab.ntb.Media.Search:input_type -> ttab.ntb.SearchRequest
	12, // 36: ttab.ntb.Media.GetItem:input_type -> ttab.ntb.GetItemRequest
	26, // 37: ttab.ntb.Media.GetSortFields:input_type -> ttab.ntb.GetSortFieldsRequest
	32, // 38: ttab.ntb.Nynorsk.Translate:input_type -> ttab.ntb.TranslateRequest
	34, // 39: ttab.ntb.Nynorsk.GetLanguagePrefs:input_type -> ttab.ntb.GetLanguagePrefsRequest
	37, // 40: ttab.ntb.Nynorsk.GetDocumentFormats:input_type -> ttab.ntb.GetDocumentFormatsRequest
	40, // 41: ttab.ntb.Nynorsk.GetPreferenceTemplates:input_type -> ttab.ntb.GetPreferenceTemplatesRequest
	8,  // 42: ttab.ntb.Metadata.GetAssignment:output_type -> ttab.ntb.GetAssignmentResponse
	11, // 43: ttab.ntb.Metadata.ListAssignments:output_type -> ttab.ntb.ListAssignmentsResponse
	16, // 44: ttab.ntb.Media.Search:output_type -> ttab.ntb.SearchResponse
	13, // 45: ttab.ntb.Media.GetItem:output_type -> ttab.ntb.GetItemResponse
	27, // 46: ttab.ntb.Media.GetSortFields:output_type -> ttab.ntb.GetSortFieldsResponse
	33, // 47: ttab.ntb.Nynorsk.Translate:output_type -> ttab.ntb.TranslateResponse
	36, // 48: ttab.ntb.Nynorsk.GetLanguagePrefs:output_type -> ttab.ntb.GetLanguagePrefsResponse
	39, // 49: ttab.ntb.Nynorsk.GetDocumentFormats:output_type -> ttab.ntb.GetDocumentFormatsResponse
	41, // 50: ttab.ntb.Nynorsk.GetPreferenceTemplates:output_type -> ttab.ntb.GetPreferenceTemplatesResponse
	42, // [42:51] is the sub-list for method output_type
	33, // [33:42] is the sub-list for method input_type
	33, // [33:33] is the sub-list for extension type_name
	33, // [33:33] is the sub-list for extension extendee
	0,  // [0:33] is the sub-list for field type_name
}

func init() { file_ntb_service_proto_init() }
func file_ntb_service_proto_init() {
	if File_ntb_service_proto != nil {
		return
	}
	file_ntb_service_proto_msgTypes[11].OneofWrappers = []any{}
	file_ntb_service_proto_msgTypes[24].OneofWrappers = []any{
		(*PreferenceValue_Enabled)(nil),
		(*PreferenceValue_Words)(nil),
	}
	file_ntb_service_proto_msgTypes[25].OneofWrappers = []any{
		(*TranslateRequest_Text)(nil),
		(*TranslateRequest_Texts)(nil),
		(*TranslateRequest_TextMap)(nil),
	}
	file_ntb_service_proto_msgTypes[26].OneofWrappers = []any{
		(*TranslateResponse_Text)(nil),
		(*TranslateResponse_Texts)(nil),
		(*TranslateResponse_TextMap)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_ntb_service_proto_rawDesc), len(file_ntb_service_proto_rawDesc)),
			NumEnums:      7,
			NumMessages:   40,
			NumExtensions: 0,
			NumServices:   3,
		},
		GoTypes:           file_ntb_service_proto_goTypes,
		DependencyIndexes: file_ntb_service_proto_depIdxs,
		EnumInfos:         file_ntb_service_proto_enumTypes,
		MessageInfos:      file_ntb_service_proto_msgTypes,
	}.Build()
	File_ntb_service_proto = out.File
	file_ntb_service_proto_goTypes = nil
	file_ntb_service_proto_depIdxs = nil
}
