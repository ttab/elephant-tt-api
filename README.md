...
### What has to be tagged first

This module is generated by tools that are themselves being released, and they
have to be tagged in one order, because each step is what makes the next one
reproducible:

1. `ttab/mage` is tagged with the `rpc` namespace, its `protoc-gen-elephant-rpc`
   pin still a pseudo-version — elephantine's own magefile imports
   `ttab/mage/rpc`, so mage cannot wait for elephantine.
2. `elephantine` is tagged, which is what gives the plugin a released version
   to be pinned to.
3. `ttab/mage` is tagged again, with the plugin pinned to that elephantine tag.
   That second tag is the first mage version whose generated output comes
   entirely from released code.
4. This module bumps to it, regenerates, and is tagged.

So a release here waits for a `ttab/mage` tag in `go.mod` — a pseudo-version
there means the generated code in the tag cannot be reproduced from released
generators, and a force-push of the branch it points at would orphan it
outright.
